...
public class ImBaseActivity extends FragmentActivity implements EasyPermissions.PermissionCallbacks {
    ...
    public static void requestCameraPermission(OnPermissionCallback onPermissionCallback) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) { //6.0系统以下
            ...
            if (EasyPermissions.hasPermissions(YXActivityMangerUtil.getTopActivity(), Manifest.permission.CAMERA)) {
                // Have permission, do the thing!
                mPermissionCallback.onPermissionsGranted(permsList);
            } else {
                // Ask for one permission
                EasyPermissions.requestPermissions(YXActivityMangerUtil.getTopActivity(), YXActivityMangerUtil.getTopActivity().getString(R.string.rationale_camera), RC_CAMERA_PERM, Manifest.permission.CAMERA);
            }
        }
    }
    ...
    public static void requestWriteAndReadPermission(OnPermissionCallback onPermissionCallback) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) { //6.0系统以下
            ...
            if (EasyPermissions.hasPermissions(YXActivityMangerUtil.getTopActivity(), perms)) {
                // Have permission, do the thing!
                mPermissionCallback.onPermissionsGranted(permsList);
            } else {
                // Ask for one permission
                EasyPermissions.requestPermissions(YXActivityMangerUtil.getTopActivity(), YXActivityMangerUtil.getTopActivity().getString(R.string.rationale_write_read), RC_WRITE_READ_PERM, perms);
            }
        }
    }
    ...
    public static void requestPermissions(String[] perms, OnPermissionCallback onPermissionCallback) {
        ...
        if (EasyPermissions.hasPermissions(YXActivityMangerUtil.getTopActivity(), perms)) {
            ...
            EasyPermissions.requestPermissions(YXActivityMangerUtil.getTopActivity(), YXActivityMangerUtil.getTopActivity().getString(R.string.rationale_other), RC_OTHER_PERM, perms);
        }
    }
